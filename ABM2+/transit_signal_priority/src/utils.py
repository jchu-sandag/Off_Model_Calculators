...
def ox_graph(nodes_df, links_df):
    ...
    graph_links["id"] = graph_links["hwycovid"]
    graph_links["key"] = graph_links["hwycovid"]
...
def get_primary_corridor_links(links_df, nodes_df, tsp_intersections_dict):
    ...
    sp_link_ids = sp_links["hwycovid"].tolist()
    ...
    other_first_intersection_links = first_intersection_links[~first_intersection_links["hwycovid"].isin(
        sp_link_ids)]
    other_last_intersection_links = last_intersection_links[~last_intersection_links["hwycovid"].isin(
        sp_link_ids)]

    add_first_link_ids = other_first_intersection_links["hwycovid"].tolist()
    add_last_link_ids = other_last_intersection_links["hwycovid"].tolist()
...
def get_secondary_links(links_df, tsp_intersections_dict):
    ...
    for node_id, corridor_name in zip(tsp_intersections_dict["intersection_node_id"], tsp_intersections_dict["mainline_corridor_name"]):
        ...
        selected_link_ids = selected_link_ids + secondary_links_df["hwycovid"].tolist()
...
def get_corridor_links(links_df, tsp_strategy_dict):
    ...
    corridor_links_df = links_df[links_df["hwycovid"].isin(corridor_link_ids)]
    corridor_links_df = corridor_links_df[["hwycovid", "link_name", "from_node", "to_node"]]
    corridor_links_df["corridor"] = np.where(corridor_links_df["hwycovid"].isin(
...
def get_average_transit_headway(primary_links_df, transit_routes_df, transit_links_df, cycle_length, period):
    ...
    work_df = pd.merge(primary_links_df, transit_links_df,
                       left_on="hwycovid", right_on="Link_ID", how="left")
...
def get_average_transit_trip_length(primary_links_df, transit_links_df, transit_flow_df, transit_onoff_df, period):
    ...
    """
    primary_links_ids = primary_links_df["hwycovid"].to_list()
...
def get_average_transit_flow(primary_links_df, transit_aggflow_df, period):
    ...
    work_df = pd.merge(primary_links_df, flow_df, left_on="hwycovid",
...
def get_corridor_transit_stops(corridor_links_df, transit_routes_df, transit_stops_df):
    ...
    work_df = pd.merge(work_df, transit_stops_df, left_on="hwycovid",
                       right_on="Link_ID", how="left")
...
def get_link_attributes(corridor_links_df, highway_load_data_dict, transit_aggflow_df):
    ...
    for period in highway_load_data_dict.keys():
        ...
        out_df = pd.merge(out_df, highway_load_df, how="left",
                          left_on="hwycovid", right_on="ID1")

        out_df = pd.merge(out_df, transit_flow_df, how="left",
                          left_on="hwycovid", right_on="LINK_ID")
